...
local function filter(checkFunc)
    local result = {}
...
local function count(checkFunc)
    return #filter(checkFunc)
end
...
local function checkCraft(recipe, iteration)
    ...
    local basePotions = filter(function(r) return r.id == recipe.basePotionId end)
...
for i, recipe in pairs(recipes) do
    ...
    local idCount = count(function(r) return r.id == recipe.id end)
    if idCount > 1 then
