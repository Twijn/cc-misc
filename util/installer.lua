...
---Create or update startup file to set up package.path for installed libraries
---@param installDir string The directory where libraries were installed
---@return boolean success Whether the startup file was created/updated
local function setupStartupFile(installDir)
    local startupPath = "startup.lua"
    local diskStartupPath = "disk/startup.lua"
    
    -- Use disk startup if installing to disk
    local targetStartup = installDir:find("^disk/") and diskStartupPath or startupPath
    local libPath = installDir:gsub("^/", "") -- Remove leading slash if present
    
    -- The code snippet to ensure package.path includes our lib directory
    local pathSetupCode = string.format([[
-- CC-Misc library path setup (auto-generated by installer)
if not package.path:find("%s") then
    package.path = package.path .. ";/%s/?.lua"
end
-- End CC-Misc library path setup
]], libPath:gsub("%-", "%%-"):gsub("/", "/"), libPath)

    logInfo("Setting up startup file: " .. targetStartup)
    
    -- Check if startup file exists
    if fs.exists(targetStartup) then
        -- Read existing content
        local f = fs.open(targetStartup, "r")
        if not f then
            logError("Could not read existing startup file: " .. targetStartup)
            return false
        end
        ...
        if content:find("CC%-Misc library path setup") then
            logInfo("Startup file already contains CC-Misc path setup")
            return true
        end
        
        -- Prepend our setup code to existing startup
        local newContent = pathSetupCode .. "\n" .. content
        local fw = fs.open(targetStartup, "w")
        if not fw then
            logError("Could not write to startup file: " .. targetStartup)
            return false
        end
        fw.write(newContent)
        fw.close()
        logInfo("Updated existing startup file with package.path setup")
    else
        -- Create new startup file
        local f = fs.open(targetStartup, "w")
        if not f then
            logError("Could not create startup file: " .. targetStartup)
            return false
        end
        f.write(pathSetupCode)
        f.close()
        logInfo("Created new startup file with package.path setup")
    end
...
local function installLibraries(libraries, installDir, skipClear, originalSelection)
    ...
    -- Set up startup file for package.path
    if success > 0 then
        print()
        term.setTextColor(colors.lightGray)
        write("Setting up startup file for require()... ")
        if setupStartupFile(installDir) then
            term.setTextColor(colors.green)
            print("OK")
            term.setTextColor(colors.cyan)
            print("Libraries can be loaded with: require(\"libraryname\")")
            print("Note: Reboot the computer for package.path changes to take effect")
        else
            term.setTextColor(colors.yellow)
            print("SKIPPED")
            term.setTextColor(colors.lightGray)
            print("You may need to manually add to package.path:")
            print("  package.path = package.path .. \";/" .. installDir .. "/?.lua\"")
        end
    end
